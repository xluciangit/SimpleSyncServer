...
const { requireSession, lookupGeoIp } = security;

logger.init(dbLib.DATA_DIR, fmtBytes, lookupGeoIp);
logger.startPruner();
...
app.use((req, res, next) => {
    const isUpload = req.path === '/api/upload' || req.path === '/api/whatsapp/upload';
    const timeout = isUpload ? 30 * 60 * 1000 : 30 * 1000;
...
app.get('/', requireSession, csrfProtection, async (req, res) => {
    ...
    const waStats    = dbLib.getWhatsAppStats(user.id);
    ...
    await renderDashboard(res, {
        ...
        waFiles: waStats.totalFiles,
        waBytes: fmtBytes(waStats.totalBytes),
        waLastBackup: waStats.lastBackup
    });
});
