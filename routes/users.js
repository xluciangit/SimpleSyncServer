...
app.post('/users/unblock', requireAdmin, csrfProtection, async (req, res) => {
    const { ip } = req.body;
    ...
    const secEvts = () => db.prepare('SELECT ip, event, created_at, country, city FROM security_events ORDER BY created_at DESC LIMIT 100').all();
    
    if (!ip) {
        return renderUsersPage(res, { 
            users: users(), 
            blocked: blocked(), 
            secEvents: secEvts(),
            theme, 
...
app.get('/users', requireAdmin, csrfProtection, async (req, res) => {
    const users = db.prepare('SELECT id, username, is_admin, created_at FROM users ORDER BY created_at ASC').all();
    const blocked = db.prepare('SELECT ip, blocked_at, reason, country, city FROM blocked_ips ORDER BY blocked_at DESC').all();
    const secEvents = db.prepare('SELECT ip, event, created_at, country, city FROM security_events ORDER BY created_at DESC LIMIT 100').all();
    const theme = req.session.theme || 'dark';
...
app.post('/users/create', requireAdmin, csrfProtection, async (req, res) => {
...
app.post('/users/delete', requireAdmin, csrfProtection, async (req, res) => {
    const { user_id, delete_db, delete_files, delete_whatsapp } = req.body;
    const theme = req.session.theme || 'dark';
    ...
    if (doFiles) {
        ...
        if (fs.existsSync(userDir)) {
            const doWhatsApp = delete_whatsapp === 'on';
            if (doWhatsApp) {
                try {
                ...
                try {
                    ...
                    for (const entry of entries) {
                        if (entry.name === 'WhatsApp') continue;
                        const entryPath = path.join(userDir, entry.name);
                        fs.rmSync(entryPath, { recursive: true, force: true });
                    }
                } catch (e) {
                    console.error(`[DELETE USER] Failed to remove files for ${userDir}:`, e.message);
                }
            }
        }
    }
    ...
    if (doFiles && delete_whatsapp === 'on') parts.push('All files deleted from disk.');
    else if (doFiles) parts.push('Files deleted from disk (WhatsApp folder preserved).');
...
app.post('/users/change-password', requireAdmin, csrfProtection, async (req, res) => {
    ...
    if (!new_password || !isStrongPassword(new_password)) {
        return res.json({ ok: false, error: 'Password must be at least 8 characters and contain uppercase, lowercase, a number, and a special character.' });
    }
    ...
    res.json({ ok: true, success: `Password for "${target.username}" updated.` });
});

app.post('/settings/integrity-check', requireSession, csrfProtection, async (req, res) => {
    if (dbLib.activeUploads.size > 0) {
...
app.post('/users/integrity-check', requireAdmin, csrfProtection, async (req, res) => {
    if (dbLib.activeUploads.size > 0) {
        const users = db.prepare('SELECT id, username, is_admin, created_at FROM users ORDER BY created_at ASC').all();
        const blocked = db.prepare('SELECT ip, blocked_at, reason, country, city FROM blocked_ips ORDER BY blocked_at DESC').all();
        const secEvents = db.prepare('SELECT ip, event, created_at, country, city FROM security_events ORDER BY created_at DESC LIMIT 100').all();
        return renderUsersPage(res, { users, blocked, secEvents, theme: req.session.theme || 'dark', error: 'Integrity check skipped — an upload is in progress. Please try again once uploads have finished.', success: null, csrfToken: csrfLib.generateToken(req), uptime: getUptime(),
        nonce: res.locals.cspNonce });
    }
