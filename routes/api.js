...
app.get('/api/config', apiAuth, (req, res) => {
    res.json({
        ...
        upload_max_bytes: getSetting('upload_max_bytes') ? parseInt(getSetting('upload_max_bytes'), 10) : null
    });
});
...
app.post('/api/upload', apiLimiter, apiAuth, upload, (req, res) => {
    ...
    const decrementUploads = () => {
        if (dbLib.activeUploads > 0) dbLib.activeUploads--;
    };
    res.on('finish', decrementUploads);
