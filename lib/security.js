...
function logSecurityEvent(ip, event) {
    try {
        const result = db.prepare('INSERT INTO security_events (ip, event) VALUES (?, ?)').run(ip || 'unknown', event);
        const rowId = result.lastInsertRowid;
        if (ip && ip !== 'unknown') {
            lookupGeoIp(ip).then(({ country, city }) => {
                if (country || city) {
                    db.prepare('UPDATE security_events SET country = ?, city = ? WHERE id = ?').run(country, city, rowId);
                }
            }).catch(() => {});
        }
    } catch {}
...
const waLimiter  = makeRateLimiter({ windowMs: 15 * 60 * 1000, max: 100000, message: 'Too many requests, please slow down.' });
...
module.exports = { getClientIp, isIpBlocked, blockIp, logSecurityEvent, lookupGeoIp,
    makeRateLimiter, loginLimiter, loginFailures, apiLimiter, waLimiter,
    requireSession, requireAdmin, apiAuth };
